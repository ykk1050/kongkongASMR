...
| `glass` | 유리구슬 | **챠랑** | 탄성 | — |
| `sponge` | 스펀지 | **뽀드득** | 탄성 | — |
| `leaf` | 낙엽 | **바스락** | 소모성 | 2회 · 2.2초 뒤 복구 |
| `bubble` | 에어캡 | **뽁** | 소모성 | 3회 · 1.8초 뒤 복구 |
| `orbeez` | 구슬볼 | **톡톡** | 소모성 | 3회 · 1.9초 뒤 복구 |
| `sand` | 모래 | **사각** | 소모성 | 2회 · 2.0초 뒤 복구 |
| `snow` | 눈 | **뽀득** | 소모성 | 2회 · 2.4초 뒤 복구 |
...
>
> 소모성 재질(`leaf`, `bubble`)을 보기 타일에 써도 안전합니다 —
> 정답으로 인정된 타일은 즉시 복구되므로 진행이 막히지 않습니다.
...
|---|---|:---:|:---:|
| `keycap` `cotton` `jelly` `wood` | 탄성 | 부서지지 않음 | — |
| `leaf` | 소모성 | 2회 | 2.2초 |
| `bubble` | 소모성 | 3회 | 1.8초 |

큰 점프(`Space`)는 한 번에 2단계를 깎습니다.
**정답으로 인정된 타일은 즉시 damage/broken 이 0으로 복구**되므로,
소모성 재질을 써도 문제 풀이가 막히지 않습니다.
