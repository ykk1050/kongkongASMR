...
- [ ] 태블릿 가로/세로 폭에서 화면 컨트롤이 문제나 보드를 가리지 않는다
- [ ] 메뉴에서 방향키 ↔ 조이스틱을 바꿀 수 있고, 새로고침해도 그대로다
- [ ] 방향키 여덟 칸이 각각 화면의 그 방향으로 조준된다 (한 번에 한 칸만 눌러서)
- [ ] 방향키를 **쉬지 않고 연달아** 눌러도 그때그때 그 방향으로 바뀐다
      (↑ 직후 → 를 누르면 ↗ 가 아니라 → 여야 한다)
- [ ] 하트가 글자 없는 성한 타일에만 뜨고, 밟으면 목숨이 하나 는다
      (`SK.Game.debug.spawnHeart()` 로 바로 확인)
- [ ] 목숨이 가득할 때는 하트가 뜨지 않는다
- [ ] 게임 오버에서 정답 풀이가 먼저 뜨고, `확인`을 눌러야 랭킹 등록이 보인다
